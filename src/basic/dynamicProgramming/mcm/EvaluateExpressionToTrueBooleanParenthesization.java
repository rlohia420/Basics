...
public class EvaluateExpressionToTrueBooleanParenthesization {
	...
	public static int countWays(int N, String S) {
		...
		for (int row[][] : dp)
			for (int col[] : row)
				Arrays.fill(col, -1);
		return evaluate(S, 0, N - 1, 1, dp);
	}

	public static int evaluate(String str, int i, int j, int isTrue, int[][][] dp) {
		if (i > j)
			return 0;
		...
		for (int k = i + 1; k <= j - 1; k = k + 2) {
			...
			if (dp[i][k - 1][1] != -1)
				...
				leftTrue = evaluate(str, i, k - 1, 1, dp);
			}
			if (dp[i][k - 1][0] != -1)
				...
				leftFalse = evaluate(str, i, k - 1, 0, dp);
			}
			if (dp[k + 1][j][1] != -1)
				...
				rightTrue = evaluate(str, k + 1, j, 1, dp);
			}
			if (dp[k + 1][j][0] != -1)
				...
				rightFalse = evaluate(str, k + 1, j, 0, dp);
			}
